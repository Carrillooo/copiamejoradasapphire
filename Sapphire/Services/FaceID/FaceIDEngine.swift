...
enum FaceIDSecurityEvent {
    ...
    /// El modelo de liveness no está disponible. Sin él no hay defensa frente a
    /// una fotografía, así que la autenticación facial se corta y se exige
    /// contraseña en lugar de dejarla pasar.
    case livenessUnavailable
}

/// Estado de carga del modelo de liveness. Distingue "todavía cargando" de
/// "no está y no va a estar", para no cortar la sesión durante el warm-up.
enum LivenessModelAvailability {
    case unknown
    case loading
    case ready
    case unavailable
...
final class CameraController: NSObject, ObservableObject, Identifiable, AVCaptureVideoDataOutputSampleBufferDelegate {
            ...
            if capturedCount < targetCount(for: pose) {
                ...
                if FaceIDModelManager.shared.antiSpoofInEffect {
                    ...
                    if spoof.rawScore < FaceIDConfig.livenessBaseThreshold {
                        DispatchQueue.main.async { self.userInstruction = "Se requiere una cara real: se ha detectado una fotografía." }
                        return
                    }
                }
    ...
    private func handleAuthentication(observation: VNFaceObservation, pixelBuffer: CVPixelBuffer) {
        ...
        if SettingsModel.shared.settings.faceIDAntiSpoofEnabled {
            ...
            // liveness, el reconocimiento por webcam es vulnerable a una
            // fotografía; los ajustes lo advierten explícitamente.
            frameIsReal = true
        }
...
final class FaceIDModelManager {
    ...
    private var livenessAvailability: LivenessModelAvailability = .unknown
    private var sharedAntiSpoofBuffer: CVPixelBuffer?

    /// Estado observable del modelo de liveness. El motor lo consulta antes de
    /// puntuar un fotograma para no autenticar nunca sin comprobación real.
    var livenessState: LivenessModelAvailability {
        lock.lock()
        defer { lock.unlock() }
    ...
    private func performLivenessModelLoad(cfg: MLModelConfiguration) {
