...
class AuthenticationManager: NSObject, ObservableObject, BLEDelegate {
    ...
    /// Controlador de la sesión de autenticación en curso, publicado para que
    /// el notch pueda mostrar la cámara en vivo.
    @Published private(set) var activeAuthController: CameraController?
    /// Fase visible de la autenticación facial en el notch.
    @Published private(set) var faceIDNotchPhase: FaceIDNotchPhase = .idle
    ...
    private var notchPhaseToken: UUID?
    ...
    func startFaceIDAuthentication() {
        ...
        self.faceIDNotchPhase = .searching
    ...
    /// Deja visible la fase final unos instantes y luego cierra la tarjeta.
    private func finishNotchPhase(_ phase: FaceIDNotchPhase, after delay: TimeInterval) {
        faceIDNotchPhase = phase
        let token = UUID()
        notchPhaseToken = token
        DispatchQueue.main.asyncAfter(deadline: .now() + delay) { [weak self] in
            guard let self, self.notchPhaseToken == token else { return }
            self.faceIDNotchPhase = .idle
            self.activeAuthController = nil
        }
    }

    func handleFaceIDAuthenticated() {
        finishNotchPhase(.unlocked, after: 1.4)
    ...
    private func handleFaceIDSecurityEvent(_ event: FaceIDSecurityEvent) {
        ...
        // La fase del notch se fija ANTES de `tearDownFaceID`, porque el cierre
        // sólo respeta las fases finales (`.unlocked` / `.failed`): si se
        // hiciera después, la tarjeta se habría cerrado ya y el motivo del
        // fallo no llegaría a verse.
        switch event {
        case .spoofLocked:
            setStatusThrottled("Face ID locked — spoof detected.")
            finishNotchPhase(.failed("Se ha detectado una suplantación"), after: 2.6)
        case .mismatchTimeout:
            setStatusThrottled("Face ID stopped — face not recognized.")
            finishNotchPhase(.failed("No se ha reconocido la cara"), after: 2.6)
        case .livenessUnavailable:
            setStatusThrottled("Face ID no disponible: falta el modelo de liveness. Usa la contraseña.")
            finishNotchPhase(.failed("Falta el modelo de liveness"), after: 2.6)
        }
    ...
    private func tearDownFaceID(result: FaceIDAuthResult = .failed) {
        ...
        // Si no hay una fase final en curso (desbloqueo o fallo), la tarjeta
        // del notch se cierra ya.
        if faceIDNotchPhase == .idle || faceIDNotchPhase == .searching || faceIDNotchPhase == .located {
            faceIDNotchPhase = .idle
