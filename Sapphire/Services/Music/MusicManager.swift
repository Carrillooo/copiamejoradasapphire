...
class MusicManager: ObservableObject {
    ...
    /// Deshace el estado optimista de reproducción.
    ///
    /// `applyPlayingState(true)` se aplica antes de enviar el comando para que la
    /// UI responda al instante, y `beginSpotifyPlayStateReconcile` lo fija
    /// durante 3 s. Si el comando falla, ese estado es una mentira: se revierte
    /// para que el sondeo real vuelva a mandar de inmediato.
    ...
    func play() async {
        ...
        // Reanudar es `play`, no `play track ""`. Las llamadas anteriores
        // enviaban una URI vacía: AppleScript recibía un comando inválido y la
        // API oficial un `{"uris":[""]}` que responde 400. En ambos casos el
        // fallo se descartaba con `_ =` y la UI se quedaba en "reproduciendo".
        var resumed = false

        if spotifyPrivateAPI.isLoggedIn {
            if await spotifyPrivateAPI.connectResume() { return }
            if spotifyAppleScript.isAppRunning() {
                resumed = await spotifyAppleScript.play()
            } else if spotifyOfficialAPI.isAuthenticated, isPremiumUser {
                if case .success = await spotifyOfficialAPI.resume() { resumed = true }
            }
        } else if spotifyOfficialAPI.isAuthenticated, isPremiumUser {
            if case .success = await spotifyOfficialAPI.resume() { resumed = true }
        ...
        if !resumed { revertOptimisticPlayState() }
    }
